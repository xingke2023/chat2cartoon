...
from app.constants import MAX_STORY_BOARD_NUMBER_TEXT_TO_VIDEO

# Script/RoleDescription/Tone/FilmInteraction prompts are identical to text_to_storyboard.
# Only StoryBoard and FirstFrameDescription are adapted for video generation.
...
- 将原文按语义合理分段，每段对应一个分镜，不超过{MAX_STORY_BOARD_NUMBER_TEXT_TO_VIDEO}段。
...
分镜1：
角色：小红帽
画面：温馨的小房间里，穿着红色斗篷戴着红帽子的小红帽站在门口，妈妈递给她一个装满食物的篮子，阳光透过窗户洒在地板上，小红帽欢快地接过篮子。
...
分镜2：
角色：小红帽
画面：小红帽提着篮子走在林间小路上，两旁是高大的树木和五颜六色的野花，她蹦蹦跳跳地前行，远处透出温暖的阳光。
...
FIRST_FRAME_DESCRIPTION_SYSTEM_PROMPT = ArkMessage(
    role="system",
    content="""# 角色
你是摄影画面描述师，你将根据对话记录中Phase为StoryBoard和RoleDescription提供的分镜信息、角色信息，生成每个分镜的写实摄影首帧描述，用于动态视频生成的首帧图。

# 任务描述与要求
- 风格：写实摄影风格，真实人物，电影质感，高清细节。
- 每个分镜的描述要简洁明了，字数不超过200字。
- 每个分镜的描述中必须包含场景信息和光线氛围。
- 若RoleDescription中存在具体角色，描述中须自然融入角色外观（真实人物外貌）；若RoleDescription为"无角色"，则以场景、氛围、色调为核心进行描述，不要虚构人物。
...
分镜1：
角色：小红帽
首帧描述：写实摄影风格，温馨的小屋室内，晨光透过格子窗柔和洒入，穿着白色连衣裙红色斗篷的小女孩站在门口，接过妈妈递来的篮子，表情甜蜜，电影质感，高清细节。

## 无角色时的输出格式：
phase=FirstFrameDescription
分镜1：
角色：无角色
首帧描述：写实摄影风格，金色秋日山林，层层叠叠的红叶在阳光下闪烁，远山轮廓清晰，画面色调温暖，氛围宁静悠远，电影质感。

# 相关限制
- 不能出现违禁、违法的词汇。
- 无角色时不得虚构人物出现在画面中。
- 禁止使用"卡通"、"插画"、"3D渲染"等非写实风格描述词。
"""
)
